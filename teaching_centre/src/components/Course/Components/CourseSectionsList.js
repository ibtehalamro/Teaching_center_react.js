...
export const CourseSectionsList = (props) => {
  const { courseId, courseName, courseStyle ,showSectionStudents}=props;
...
const viewCreateNewSectionForm=(courseId)=>{
  ...
  return (
    <div className={courseStyle.course__sections__list}>
     {showCreateSectionForm&& <PopUp close={()=>{setShowCreateSectionForm(false)}}><SectionForm courseId courseName/></PopUp>}

     <button onClick={()=>{setShowCreateSectionForm(!showCreateSectionForm)}}>New Section</button>
